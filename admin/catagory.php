                    ...
                    <div class="container px-4 px-lg-5 mt-4">
                        <div class="row">
                            ...
                            <div class="col-6 d-flex justify-content-end ">
                                <button class="btn btn-outline-primary"><a class="text text-decoration-none" href="addcatagory.php">เพิ่มหมวดหมู่</a></button>
                            </div>
                        </div>
                            ...
                            <div class="card-body">
                                <table id="datatablesSimple">
									...
									?>
										<tr >
                                            <td>
                                            <div class="row p-1" style="cursor:pointer;" onclick="window.location.href='products_catagory.php?id=<?= $Cattab['id'] ?>'">
                                                <?=$Cattab['cat_name']?></td>
                                            </div>    
                                            <?php
                                                $cat = $sql->count_cat_product($Cattab['id']);
                                                while($Cat=mysqli_fetch_array($cat)){
                                            ?>
                                            <td>
                                            <div class="row p-1" style="cursor:pointer;" onclick="window.location.href='products_catagory.php?id=<?= $Cattab['id'] ?>'">
                                                <?=$Cat['count_cat_product']?>
                                            </div>
                                            </td>
                                            <?php
                                                }
                                            ?>
                                            <td>
                                                <a class="text-decoration-none" href="editcatagory.php?id=<?= $Cattab['id'] ?>">
                                                    <button type="button" style="width:130px; height:60; font-size:17px;" class="btn btn-outline-primary">แก้ไขข้อมูล</button>
                                                </a>
                                                <a class="text-decoration-none" href="products_catagory.php?id=<?= $Cattab['id'] ?>">
                                                    <button type="button" style="width:130px; height:60; font-size:17px;" class="btn btn-primary">ดูสินค้า</button>
                                                </a>
                                                <a class="text-decoration-none" href="delete.php?cat_id=<?=$Cattab['id']?>&what=catagory">   
                                                    <button type="button" style="width:130px; height:60; font-size:17px;" class="btn btn-danger">ลบหมวดหมู่</button>
                                                </a>
                                            </td>
                                        </tr>
									<?php
